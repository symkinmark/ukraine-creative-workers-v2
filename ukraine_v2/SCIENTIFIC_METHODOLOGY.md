...
| 2.14 | 2026-04-10 | Hybrid restructure: Introduction flowed into single block (no subsections), Results merged into single §4 (findings + robustness together), Limitations moved inside Discussion. 16 main-body figures. Deep audit: fixed stale Cox HRs (0.759→0.777, 5.396→4.403), stale CONSORT counts, stale OLS betas, wrong V1 numbers (5.4→9.0), missing workers list corrected (7 workers, mean 46.7), bibliography cleaned, PH violation disclosure added, checker expanded to 184 checks |
| 2.15 | 2026-04-12 | Dataset cleaned: excluded 1 film title entry ("Два кольори", not a person) and 4 exact duplicate entries. N: 8,590→8,585. All statistics, charts, and Cox analyses regenerated. 184/184 checks pass |

---
> **V3.0 CURRENT (cleaned 2026-04-12).** Primary dataset: `esu_creative_workers_v2_6.csv`. Analysable dead cohort: **N=8,585** (migrated=1,323 | non_migrated=5,957 | internal_transfer=1,111 | deported=194). Primary gap: **3.98 years** (Cohen's d=0.292). All 184 numeric claims verified. Dataset cleaned: 1 film title and 4 exact duplicates excluded. Paper uses progressive-revelation structure: §4 core gap → §5 patterns → §6 robustness. 15 main-body figures + 18 in Appendix A. See AI_METHODOLOGY_LOG.md Stages 11–15 for full log.
>
