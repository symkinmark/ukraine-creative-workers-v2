...
**Validation of AI-assisted classification.** To assess classification accuracy, a random sample of 63 entries (seed=99) was drawn from the classified dataset and reviewed by the author against the pre-specified classification criteria. Of the 62 entries for which a definitive verdict could be reached, two were incorrectly classified, yielding an **observed error rate of 3.2%** (95% CI: approximately 0.9%–7.9%, exact binomial). Both errors were edge cases rather than systematic failures: one nationality determination, one migration status — neither in the direction of favouring the migrated group.

This validation deviates from the pre-specified procedure (SCIENTIFIC_METHODOLOGY.md §6.6, §13.1) in three ways that limit its precision. First, the sample is n=62 against a recommended n=200. Second, sampling was unstratified — the recommended design called for 100 INCLUDE and 100 EXCLUDE entries sampled separately to yield false positive and false negative rates; the implemented sample was a single random draw, so category-specific error rates cannot be computed. Third, the review was conducted by the study author rather than an independent reviewer. As a consequence, the 3.2% figure is a directional estimate sufficient to establish that the error rate is likely below 5% and to power the sensitivity analysis, but it does not constitute publication-grade validation. A full stratified independent validation (n=200, reviewer independent of the study) is a defined deliverable for V3 prior to journal submission.
