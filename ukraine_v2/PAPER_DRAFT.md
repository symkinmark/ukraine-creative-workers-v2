...
Non-migrants who remained within the USSR (n=6,030) achieved an average age at death of 71.22 years (95% CI: 70.87–71.57; median 73, SD 13.79), while migrants who left the Soviet sphere (n=1,280) achieved an average of 75.25 years (95% CI: 74.49–76.01; median 77, SD 13.88), yielding a **gap of 4.04 years** in favour of emigrants (Mann-Whitney U, p<0.001; Cohen's d=0.292; Cliff's δ=0.18).[^8] As discussed in §5.4, this gap is subject to healthy migrant selection effects; propensity score matching on birth decade, profession, and region reduces the gap to 3.35 years (95% CI [2.26, 4.45]), suggesting it is not primarily an artefact of observable pre-migration differences. Cox proportional hazards analysis (§4.10) on the full right-censored dataset (N=15,053) confirms the OLS regression findings: migrated workers show HR = 0.832 (lower mortality hazard) and deported workers show HR = 4.646 (far higher mortality hazard) relative to non-migrants after covariate adjustment.
...
We approach this question with awareness of its political sensitivity. This is an observational study; we document associations, not causes. We acknowledge the significant selection effects inherent in emigration — and note that the study's most causally secure findings, the deportee mortality catastrophe and the internal transfer null, are not subject to emigration selection bias at all (§5.4). We situate our quantitative findings within the documented historical record of Soviet policy toward Ukrainian cultural workers, rather than treating the numbers as speaking for themselves.
...
Two Cox proportional hazards models were estimated on the extended right-censored dataset (N=15,053). This includes both confirmed deaths (n=8,739, event_observed=1) and 6,314 right-censored living individuals (event_observed=0, censored at age = 2026 − birth year). Unlike the OLS analysis, the Cox model can accommodate censored observations without excluding them. The migration status of living individuals was classified using the same AI pipeline (Claude Haiku/Sonnet) applied to the dead cohort, enabling proper distribution of censoring across all groups rather than concentrating it artificially in non-migrated. Model 1 is unadjusted; Model 2 controls for birth decade, profession, and birth region.

**Table A-Cox — Cox PH Results, Right-Censored Model (reference = non-migrated, N=15,053)**

| Group | Model 1 HR | Model 2 HR | 95% CI (Model 2) | p (Model 2) |
|-------|-----------|-----------|-----------------|-------------|
...
*HR < 1 = lower hazard of death = longer survival. HR > 1 = higher hazard = shorter survival. Reference: non-migrated. Full model output in `results/cox_censored_output.txt`. Concordance (Model 2): 0.724.*

A noteworthy feature of this table is the reversal between Model 1 and Model 2 for the migrated group: the unadjusted HR is 1.088 (slightly elevated) but the adjusted HR is 0.832 (protective). This reversal is a direct consequence of differential censoring — the non-migrated group has 48.9% censoring (living individuals) versus 19.8% for migrants, which artificially lowers the non-migrated baseline hazard in unadjusted analysis. Once birth cohort, profession, and region are controlled, the survival advantage of migrants is restored. This demonstrates methodologically why covariate adjustment is essential in right-censored survival analysis when censoring is differentially distributed across groups.

After adjustment, results are directionally consistent with the OLS regression: migrated workers show approximately 17% lower mortality hazard (HR = 0.832); deported workers show more than four-fold higher hazard (HR = 4.646); the internal transfer group shows a small elevated hazard (HR = 1.105, p = 0.004). The deported HR is somewhat lower than the complete-case estimate (5.40 in V2.3) because the right-censored model includes living deported-cohort descendants in the baseline, diluting the catastrophic excess mortality signal.

**Figure 24.** Cox PH forest plot — hazard ratios for migration groups relative to non-migrated (HR=1.0, dashed line), for both Model 1 (squares) and Model 2 (diamonds). Error bars = 95% CI. The migrated reversal between models (1.09→0.83) is clearly visible. N=15,053.

**Schoenfeld residuals test (PH assumption).** The PH assumption is violated for the deported group (p < 0.0001) and borderline for migrated (p = 0.011), indicating that hazard ratios are not constant across age. This is expected: deported workers experienced concentrated mortality in 1937–1945 (Gulag deaths, executions), creating a time-varying hazard pattern not captured by a single HR. Reported HRs represent average effects over the full follow-up period. Internal transfer is within the acceptable range (p = 0.066).

**Censoring distribution.** With living individuals properly classified across all migration groups, the structural censoring imbalance of V2.3 is resolved. Censoring rates: non-migrated 48.9%, migrated 19.8%, internal transfer 11.7%, deported 6.1% (see Figure 25).

**Figure 25.** Censoring pattern by migration group: proportion of dead (event observed) vs alive/right-censored in the extended dataset (N=15,053). Unlike V2.3, censoring is now distributed across all groups, enabling valid right-censored Cox inference. Non-migrated has the highest censoring rate (48.9%) because it is the largest group; all other groups have meaningful living cohorts.

**Figure 26.** Kaplan-Meier survival curves using the full right-censored dataset (N=15,053). Tick marks on curves indicate right-censored observations (living individuals). All groups now show censoring tick marks. The deported group's sharp early decline (1937–1945 Gulag deaths) is unchanged.

### 4.11 Sensitivity Analyses

Three sensitivity scenarios were tested on the right-censored model (all using unadjusted Model 1 for computational tractability; see §4.10 for the adjusted estimate). All results are summarised in Figure 27.

**Scenario A — Implausibly alive age assumption.** The 185 individuals born before 1920 with no recorded death date are implausibly coded as alive in 2026 (106+ years old). These were reclassified as deceased with assumed ages at death of 70, 75, 80, 85, and 90 years. The migrated HR ranges from 1.100 (age=70) to 1.067 (age=90) across all scenarios — consistent direction throughout, confirming the finding is not sensitive to this assumption.

**Scenario B — Post-Soviet emigrant classification.** Sixty-two living migrants born after 1960 were flagged as likely post-Soviet emigrants (emigration after 1991 rather than during the Soviet period). Three sub-scenarios were tested: include all (HR=1.088), exclude the 62 individuals from the migrated group entirely (HR=1.094), or reclassify them as non-migrated (HR=1.094). The maximum change is 0.006 HR units — negligible. The finding is not sensitive to the classification of post-Soviet emigrants.

**Scenario C — Bootstrap misclassification error.** To assess the robustness of migration status classifications (AI-assigned, ~3.2% validated error rate), 50-iteration bootstraps were run at 5%, 10%, and 15% random misclassification rates. Migrated median HR: 5% → 1.088 (2.5–97.5%: 1.064–1.109), 10% → 1.077 (1.048–1.110), 15% → 1.083 (1.050–1.111). The direction and approximate magnitude are stable across all misclassification rates tested.

The unadjusted migrated HR > 1 in all sensitivity scenarios reflects the censoring imbalance discussed above; the adjusted Model 2 HR of 0.832 is the appropriate primary estimate. The sensitivity analyses confirm that this adjusted estimate is not driven by edge-case classification decisions.

**Figure 27.** Sensitivity analysis summary — migrated HR (95% CI) across the baseline and all three sensitivity scenarios. Dashed line at HR=1.0; dotted line at baseline HR=0.832. The direction of all unadjusted estimates (HR > 1) is consistent with the structural censoring argument; the adjusted baseline (HR=0.832) represents the corrected estimate.
...
This paper presents the most comprehensive quantitative analysis to date of observed age at death among Ukrainian creative workers under Soviet occupation. Across a dataset of 8,643 individuals with confirmed birth dates, death dates, and migration status — more than twenty times larger than our V1 dataset — three findings stand out.

The most causally interpretable findings are not subject to emigration selection effects. Workers **deported by the Soviet state** died at a mean age of 48.35 years — 22.87 years below non-migrants (d=1.656, HR=5.40 in the adjusted Cox model, p<0.001). Workers who **transferred voluntarily within the Soviet system** showed no survival advantage over those who stayed (+0.52 yrs, p=0.094, HR≈1.08). These two results point in the same direction: the relevant variable was not mobility but Soviet jurisdiction. Moving within the Soviet system — whether forced or voluntary — provided no escape from its mortality conditions. Only leaving the Soviet sphere entirely was associated with longer life.

Additionally, those who emigrated lived on average **4.04 years longer** than non-migrants (Cohen's d=0.292, p<0.001). Propensity score matching on birth decade, profession, and region reduced this gap to 3.35 years (95% CI [2.26, 4.45]), confirming it is not primarily an artefact of observable compositional differences. Cox proportional hazards analysis yields HR=0.76 for migrated workers — 24% lower mortality hazard than non-migrants after covariate adjustment. Selection effects that cannot be fully addressed with these data remain a limitation (§5.4).
